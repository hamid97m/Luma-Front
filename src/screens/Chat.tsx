  ...
  return (
    <div className="flex flex-col h-full bg-bg text-txt" style={{ paddingTop: 'var(--tg-safe-top)' }}>
      ...
      {loadState === 'unavailable' ? (
        <div className="flex-1 flex items-center justify-center p-8 text-center text-txt2 text-[15px]">
          {t.chat.unavailable}
        </div>
      ) : loadState === 'error' ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-3 p-8 text-center">
          <p className="text-txt2 text-[15px]">{t.chat.loadError}</p>
          <Button variant="tonal" onClick={() => { setLoadState('loading'); load() }}>
            {t.chat.retry}
          </Button>
        </div>
      ) : (
            ...
            <div className="relative flex-1 flex flex-col overflow-hidden">
              ...
              {showJump && (
                <button
                  ...
                  className="absolute bottom-3 right-4 w-10 h-10 rounded-full bg-surface text-txt flex items-center justify-center shadow-m3-1"
                >
